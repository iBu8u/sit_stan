data {
  ...
  real<lower=0,upper=1>  wgtWith_one[nSubjects,nTrials];       
  real<lower=0,upper=1>  wgtAgst_one[nSubjects,nTrials];   
...
model {
  ...
  for (s in 1:nSubjects) {
    ...
    for (t in 1:nTrials) {
      ...
      valfun2 <- beta[3,s] + beta[4,s]*valdiff + beta[5,s]*wgtWith_one[s,t] + beta[6,s]*wgtAgst_one[s,t];
      ...
      if (t==1) {
        otherValue[t+1,choice2[s,t]]   <- sum(othW .* otherReward2[s,t]);
        otherValue[t+1,3-choice2[s,t]] <- sum((1-othW) .* otherReward2[s,t]);
      } else if (t==2) {
        otherValue[t+1,choice2[s,t]]   <- sum(othW .* otherReward2[s,t]     + othW .* otherReward2[s,t-1]*disc[s]);
        otherValue[t+1,3-choice2[s,t]] <- sum((1-othW) .* otherReward2[s,t] + (1-othW) .* otherReward2[s,t-1]*disc[s]);
      } else {
        disc_mat <- rep_matrix(exp(log(disc[s])*pwr),4);
        otherValue[t+1,choice2[s,t]]   <- sum(disc_mat .* rep_matrix(othW,3)    .* block(otherReward2[s], t-2, 1, 3, 4) );
        otherValue[t+1,3-choice2[s,t]] <- sum(disc_mat .* rep_matrix(1-othW,3)  .* block(otherReward2[s], t-2, 1, 3, 4) );
      }
    }  // trial loop
  }    // subject loop
}
...
generated quantities {
  ...
  for (s in 1:nSubjects) {
    ...
    for (t in 1:nTrials) {
      ...
      valfun2_gen  <- beta[3,s] + beta[4,s]*valdiff_gen + beta[5,s]*wgtWith_one[s,t] + beta[6,s]*wgtAgst_one[s,t];
      ...
      if (t==1) {
        otherValue2[t+1,choice2[s,t]]   <- sum(othW2 .* otherReward2[s,t]);
        otherValue2[t+1,3-choice2[s,t]] <- sum((1-othW2) .* otherReward2[s,t]);
      } else if (t==2) {
        otherValue2[t+1,choice2[s,t]]   <- sum(othW2 .* otherReward2[s,t]     + othW2 .* otherReward2[s,t-1]*disc[s]);
        otherValue2[t+1,3-choice2[s,t]] <- sum((1-othW2) .* otherReward2[s,t] + (1-othW2) .* otherReward2[s,t-1]*disc[s]);
      } else {
        disc_mat2 <- rep_matrix(exp(log(disc[s])*pwr),4);
        otherValue2[t+1,choice2[s,t]]   <- sum(disc_mat2 .* rep_matrix(othW2,3)   .* block(otherReward2[s], t-2, 1, 3, 4) );
        otherValue2[t+1,3-choice2[s,t]] <- sum(disc_mat2 .* rep_matrix(1-othW2,3) .* block(otherReward2[s], t-2, 1, 3, 4) );
      }
    }  // trial loop
  }    // subject loop
}