run_model <- function(modelStr, runPara=FALSE, fitOBJ=NA) {
  ...
  if (modelStr == "RevLearn_RL" || modelStr == "RevLearn_RLnc" || modelStr == "RevLearn_RLnc_cfa" || 
      modelStr == "RevLearn_RLnc_2lr" || modelStr == "RevLearn_RLnc_2lr_cfa") {
    
    choice <- array(0,dim = c(ns,nt)); reward <- array(0,dim = c(ns,nt))
    choice <- t(data[,10,])  # 1 OR 2
    reward <- t(data[,14,])  # 1 OR -1

    dataList$choice <- choice; dataList$reward <- reward
  
  } else if (modelStr == "RevLearn_RLcoh" || modelStr == "RevLearn_RLcoh_cfa" || 
    ...
    choice1 <- array(0,dim = c(ns,nt)); choice2 <- array(0,dim = c(ns,nt))
    ...
    choice1 <- t(data[,3,]);  choice2 <- t(data[,10,])
    bet1    <- t(data[,13,]); bet2    <- t(data[,19,])
    reward  <- t(data[,14,]); chswtch <- t(data[,5,])
    
    for (s in 1:ns) {
      for (tr in 1:nt){
        my1 <- data[tr,3,s]; other1 <- data[tr,6:9,s]
    ...
    dataList$choice1 <- choice1; dataList$choice2 <- choice2
    dataList$bet1    <- bet1;    dataList$bet2    <- bet2
    dataList$with    <- with;    dataList$against <- against
    dataList$reward  <- reward;  dataList$chswtch <- chswtch
    
  } else if (modelStr == "RevLearn_RLcoh_sepv_cate" || modelStr == "RevLearn_RLcoh_sepv_bern") { 
