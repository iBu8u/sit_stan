  ...
  for (s in 1:nSubjects) {
    ...
    for (t in 1:nTrials) {
      if (t == 1) {
        ...
        valfun1[choice2[s,t-1]]   <- beta[1,s] * myValue[t, choice2[s,t-1]]   + beta[2,s] * otherValue[t, choice2[s,t-1]];
        valfun1[3-choice2[s,t-1]] <- beta[1,s] * myValue[t, 3-choice2[s,t-1]] + beta[3,s] * otherValue[t, 3-choice2[s,t-1]];
      }

      choice1[s,t] ~ categorical_logit( valfun1 );

      valdiff <- myValue[t,choice1[s,t]] - myValue[t,3-choice1[s,t]];
      valfun2 <- beta[4,s] + beta[5,s]*valdiff + beta[6,s]*wgtWith[s,t] + beta[7,s]*wgtAgst[s,t];
  ...
  for (s in 1:nSubjects) {
    ...
    for (t in 1:nTrials) {
      if (t == 1) {
        ...
        valfun1_gen[choice2[s,t-1]]   <- beta[1,s] * myValue2[t, choice2[s,t-1]]   + beta[2,s] * otherValue2[t, choice2[s,t-1]];
        valfun1_gen[3-choice2[s,t-1]] <- beta[1,s] * myValue2[t, 3-choice2[s,t-1]] + beta[3,s] * otherValue2[t, 3-choice2[s,t-1]];
      }
      ...
      valfun2_gen  <- beta[4,s] + beta[5,s]*valdiff_gen + beta[6,s]*wgtWith[s,t] + beta[7,s]*wgtAgst[s,t];
