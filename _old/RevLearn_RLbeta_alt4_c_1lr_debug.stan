...
model {
  ...
  for (s in 1:nSubjects) {
    ...
    for (t in 1:nTrials) {
      ...
      //print("valfun1=", valfun1, " myValue = ", myValue[t], " otherValue = ", otherValue[t], " disc[s] = ", disc[s], " lr[s] = ", lr[s]);
      ...
      wght <- to_row_vector(wOthers[nSubjects,nTrials]);
      othW <- to_row_vector(otherWith2[nSubjects,nTrials]); 
      ...
      if (t==1) {
        ...
        otherValue[t+1,choice2[s,t]]   <- sum(wght .* othW .* otherReward2[s,t]     + wght .* othW .* otherReward2[s,t-1]*disc[s]     + wght .* othW .* otherReward2[s,t-2]*disc[s]);
        ...
        otherValue[t+1,choice2[s,t]]   <- sum(disc_mat .* rep_matrix(wght,3) .* rep_matrix(othW,3)    .* block(otherReward2[s], t-2, 1, 3, 4) );
        otherValue[t+1,3-choice2[s,t]] <- sum(disc_mat .* rep_matrix(wght,3) .* rep_matrix(1-othW,3)  .* block(otherReward2[s], t-2, 1, 3, 4) );
        //print("disc_mat", disc_mat, " wght", rep_matrix(wght,3), " oth" , rep_matrix(othW,3), " rew" , block(otherReward2[s], t-2, 1, 3, 4) )
      }
    }  // trial loop
  }    // subject loop
}
...
generated quantities {
  ...
  for (s in 1:nSubjects) {
    ...
    for (t in 1:nTrials) {
      ...
      wght2 <- to_row_vector(wOthers[nSubjects,nTrials]);
      othW2 <- to_row_vector(otherWith2[nSubjects,nTrials]); 
      ...
      if (t==1) {
        ...
        otherValue2[t+1,choice2[s,t]]   <- sum(wght2 .* othW2 .* otherReward2[s,t]     + wght2 .* othW2 .* otherReward2[s,t-1]*disc[s]     + wght2 .* othW2 .* otherReward2[s,t-2]*disc[s]);
        ...
        otherValue2[t+1,choice2[s,t]]   <- sum(disc_mat2 .* rep_matrix(wght2,3) .* rep_matrix(othW2,3)   .* block(otherReward2[s], t-2, 1, 3, 4) );
        otherValue2[t+1,3-choice2[s,t]] <- sum(disc_mat2 .* rep_matrix(wght2,3) .* rep_matrix(1-othW2,3) .* block(otherReward2[s], t-2, 1, 3, 4) );
      }
    }  // trial loop
  }    // subject loop
}