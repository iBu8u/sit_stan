...
parameters {
  ...
  vector<lower=L,upper=U>[nSubjects] coh;
}
...
model {
  ...
  for (s in 1:nSubjects) {
    ...
    coh[s] ~ normal(coh_mu,coh_sd) T[L,U];
    
    v[1] <- rep_vector(0.0,2);
    
    for (t in 1:obs[s+1] ) {
      
      v[t][3-choice1[t+cumobs[s]]] <- v[t][3-choice1[t+cumobs[s]]] + coh[s] * against[t+cumobs[s]]*4;
      v[t][choice1[t+cumobs[s]]]   <- v[t][choice1[t+cumobs[s]]]   + coh[s] * with[t+cumobs[s]]*4;
...
generated quantities {
  ...
  for (s in 1:nSubjects) {
    ...
    for (t in 1:obs[s+1] ) {
      v2[t][3-choice1[t+cumobs[s]]] <- v2[t][3-choice1[t+cumobs[s]]] + coh[s] * against[t+cumobs[s]]*4;
      v2[t][choice1[t+cumobs[s]]]   <- v2[t][choice1[t+cumobs[s]]]   + coh[s] * with[t+cumobs[s]]*4;
