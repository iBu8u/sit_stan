data {
  ...
  matrix<lower=0.25,upper=1>[nTrials,4] wOthers[nSubjects];     // others' weight based on pref
  matrix<lower=0,upper=5>[nTrials,4]    cfsC2[nSubjects];          // cum-window freq, same as my C2
  matrix<lower=0,upper=5>[nTrials,4]    cfoC2[nSubjects];          // cum-window freq, same as my C2
}
...
model {
  ...
  vector[2] valfun1;
  real valfun2;
  row_vector[4] prob_sC2;  // others' prob of choosing the same as my C2
  row_vector[4] prob_oC2;  // others' prob of choosing the opposite as my C2
  row_vector[4] wProb_sC2; // weighted prob based on preference' weight, same as my C2
  row_vector[4] wProb_oC2; // weighted prob based on preference' weight, opposite to my C2
  row_vector[4] evid_sC2;  // evidW * cfsC2
  row_vector[4] evid_oC2;  // evidW * cfoC2
  ...
  for (s in 1:nSubjects) {
    ...
    for (t in 1:nTrials) {
      ...
      evid_sC2 <- evidW[s] * cfsC2[s,t];
      evid_oC2 <- evidW[s] * cfoC2[s,t];
      
      for (o in 1:4) {
        // [note] the [a b] parameter of beta_cdf must be positive, and count from beta(1,1)
        prob_sC2[o]  <- beta_cdf(0.5, evid_oC2[o] + 1, evid_sC2[o]+ 1 );
      }
      prob_oC2  <- 1 - prob_sC2;
      wProb_sC2 <- wOthers[s,t] .* prob_sC2;
      wProb_oC2 <- wOthers[s,t] .* prob_oC2;
...
generated quantities {
  ...
  real valfun2_gen;
  row_vector[4] prob_sC2_gen;
  row_vector[4] prob_oC2_gen;
  row_vector[4] wProb_sC2_gen;
  row_vector[4] wProb_oC2_gen;
  row_vector[4] evid_sC2_gen;
  row_vector[4] evid_oC2_gen;
  ...
  for (s in 1:nSubjects) {
    ...
    for (t in 1:nTrials) {
      ...
      evid_sC2_gen <- evidW[s] * cfsC2[s,t];
      evid_oC2_gen <- evidW[s] * cfoC2[s,t];
      
      for (o in 1:4) {
        // [note] the [a b] parameter of beta_cdf must be positive, and count from beta(1,1)
        prob_sC2_gen[o]  <- beta_cdf(0.5, evid_oC2_gen[o] + 1, evid_sC2_gen[o]+ 1 );
      }

      prob_oC2_gen  <- 1 - prob_sC2_gen;
      wProb_sC2_gen <- wOthers[s,t] .* prob_sC2_gen;
      wProb_oC2_gen <- wOthers[s,t] .* prob_oC2_gen;
