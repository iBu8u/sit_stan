...
parameters {
  ...
  real<lower=L,upper=U> coha[nSubjects];
  real<lower=L,upper=U> cohw[nSubjects];
}
...
model {
  ...
  for (s in 1:nSubjects) {
    ...
    for (t in 1:obs[s+1] ) {
      ...
      //print("s=",s,",t=",t, ",ini v=", v[t])
      
      v[t][3-choice1[t+cumobs[s]]] <- v[t][3-choice1[t+cumobs[s]]] + coha[s] * against[t+cumobs[s]];
      v[t][choice1[t+cumobs[s]]]   <- v[t][choice1[t+cumobs[s]]]   + cohw[s] * with[t+cumobs[s]];
      
      //print("coha_mu=", coha_mu, ",cohw_mu=", cohw_mu, ",coha_sd=", coha_sd, ",cohw_sd=", cohw_sd)
      //print("coha=",coha[s], ",cohw=",cohw[s])
...
generated quantities {
  ...
  for (s in 1:nSubjects) {
    ...
    for (t in 1:obs[s+1] ) {
      v2[t][3-choice1[t+cumobs[s]]] <- v2[t][3-choice1[t+cumobs[s]]] + coha[s] * against[t+cumobs[s]];
      v2[t][choice1[t+cumobs[s]]]   <- v2[t][choice1[t+cumobs[s]]]   + cohw[s] * with[t+cumobs[s]];
