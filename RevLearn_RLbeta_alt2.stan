data {
  ...
  real<lower=1,upper=2>  otherChoice[nSubjects,nTrials,4];
  real<lower=-1,upper=1> otherReward[nSubjects,nTrials,4];
  real<lower=1,upper=4>  pref[nSubjects,nTrials,4];        // preference for the others
...
transformed parameters {
  ...
  for (i in 1:2) {
    evid_wght[i] <- evid_wght_mu[i] + evid_wght_sd[i] * evid_wght_raw[i];
  }
  for (i in 1:11) {
...
model {
  ...
  // hyperparameters
  lr_mu_pr      ~ normal(0,1);
  evid_wght_mu  ~ normal(0,1);
  beta_mu       ~ normal(0,1);
  
  lr_sd         ~ cauchy(0,5);
  evid_wght_sd  ~ cauchy(0,5);
  beta_sd       ~ cauchy(0,5);
  
  // Matt Trick
  for (i in 1:2) {
    lr_raw[i]        ~ normal(0,1);
    evid_wght_raw[i] ~ normal(0,1);
  }
  for (i in 1:11) {
